            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    case .failure(let msg):
                        AnyView(
                            DataUnavailableView(message: msg) {
                                Task {
                                    marketVM.state = .loading
                                    do {
                                        try await marketVM.loadAllData()
                                    } catch {
                                        marketVM.state = .failure("Could not load market data: \(error.localizedDescription)")
                                    }
                                }
                            }
                        )
                    ...
                    case .success(let coins):
                        ScrollView(showsIndicators: false) {
                            LazyVStack(spacing: 0) {
                                ForEach(Array(coins), id: \.id) { coin in
