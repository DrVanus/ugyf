...
struct MarketCoin: Identifiable, Codable {
    ...
    /// Backward-compatible alias for circulating supply
    var circulatingSupplyValue: Double { circulatingSupply ?? 0 }

    /// Backward-compatible alias for 24h high
    var high24hValue: Double { high24h ?? 0 }

    /// Backward-compatible alias for 24h low
    var low24hValue: Double { low24h ?? 0 }

    /// Backward-compatible alias for max supply
    var maxSupplyValue: Double? { maxSupply }
